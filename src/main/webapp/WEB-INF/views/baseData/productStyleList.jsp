...
<body>
	<div id="formsContent">
		<table id="materialTable" class="listTable" border="1">
			...
			<tbody>
				<c:forEach items="${list}" var="productStyle">
					<tr>
						<td>${productStyle.number}</td>
						<td>${productStyle.name}</td>
						<td>${productStyle.product.name}</td>
						<td>${productStyle.remark}</td>
						<td>
							<a href="<c:url value="showAddPage"/>">新增</a>
							<a href="<c:url value="doDelete?id=${productStyle.id}"/>">删除</a>
							<a href="<c:url value="doEdit?id=${productStyle.id}"/>">修改</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
